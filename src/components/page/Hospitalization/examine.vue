                ...
                <div class="right-ctn">         
                    <div v-show="leftBck == 0">
                        ...
                        <el-table ref="multipleTable" :data="hostpitalList" highlight-current-row height="460" border tooltip-effect="dark" style="width: 100%"
                            ...
                            <el-table-column ref="quanxuan" type="selection" :selectable='checkboxInit' width="55">
                            </el-table-column>
                      ...
                      <!-- <el-button type="primary" size="mini" class="plus" icon="el-icon-zoom-in" @click="plusTable"></el-button> -->
                       ...
                       <div v-show="tableShow">
                          ...
                          <el-table ref="multipleTable" :data="moneyDataList.slice((dragPage-1)*pageSize,dragPage*pageSize)" height="460" :header-cell-style="rowClass"  border tooltip-effect="dark" style="width: 100%">
                            <el-table-column prop="thirdName" label="项目名称" width="160px" show-overflow-tooltip>
                            </el-table-column>
                    ...
                    <div class="bom-content" v-show="hasMsg">
                       <!--  <div class="exAmount">
                            <div class="exOther">
                                ...
                                <span>扣款总金额: {{deduction}}</span><br>
                            </div>
                         
                            <div class="exOther">
                              <span>药品: {{viewLeftList.drugfee -0 |rounding}}</span><br>
                              <span>医疗服务: {{viewLeftList.servicefee -0 |rounding}}</span><br>
                              <span>材料: {{viewLeftList.materialfee -0 |rounding}}</span><br>
...
export default {
  ...
  watch: {
    itemSearch() {
      /*  this.itemList = this.itemSearch? this.itemListAll.filter(item=>item.includes(this.itemSearch)):this.itemListAll
      */
      // console.log(this.itemSearch);

      if (this.itemSearch != "") {
        let arr = [];
        // var reg = /(this.itemSearch)+/g;
        var reg = new RegExp(this.itemSearch, "g");
        for (let i = 0; i <= this.itemList.length; i++) {
          // console.log(this.itemList[i])
          // if (this.itemList[i].indexOf(this.itemSearch) > -1) {
          // if (reg.exec(this.itemList[i]) != null) {
  ...
  methods: {
    ...
    // 页面初始化请求参数
    gitHospitlaMsg(rn) {
      // localStorage.setItem("hometext", "【住院-审核】");
        ...
        .then(res => {
          if (res.data.code == 0) {
            ...
            for (let i = 0; i < this.hostpitalList.length; i++) {
              ...
              /*   if(this.hostpitalList[i].status == 1) {          
                this.hostpitalList.splice(i);
                // console.log(i)
              } */
              // 机审结果的判断
