...
export default {
  ...
  methods: {
    getname() {
      localStorage.setItem("hometext", "【慢病补偿情况分析（年度）】");
      ...
      axios({
        ...
        if (res.data.code == 0) {
          ...
          for (var i = 0; i < echarts_date.length; i++) {
            ...
            datamzzrenci[i] = echarts_date[i].慢病总人次;
            datamzzrenshu[i] = echarts_date[i].慢病总人数;
            datazylfeiyong[i] = echarts_date[i].慢病总医疗费用;
            datazrjfeiyong[i] = echarts_date[i].慢病人均费用;
            dataypfeiyong[i] = echarts_date[i].慢病药品费用;
            datazlfeiyong[i] = echarts_date[i].慢病诊疗费用;
            datazffeiyong[i] = echarts_date[i].慢病自费费用;
            datakbjine[i] = echarts_date[i].慢病可报金额;
            databxjine[i] = echarts_date[i].慢病报销金额;
            datafdrs[i] = echarts_date[i].慢病到达封顶人数;
          }
