<template>
   <div>
      <div class="right-ctn-title">
            ...
            <!-- <span>违规等级：</span> -->
            <el-select v-model="illegal" @change="Chartselect" size="mini" placeholder="请选择图表项目">
                <el-option
            ...
            <el-button size="mini" @click="getClistatic" type="warning" style="margin-left: 5px;">
      ...
      <div class="chart">
        ...
        <div id="cliContainter" :style="{width: '100%', height: '470px'}"></div>
      </div>
   </div>
...
export default {
  ...
  methods: {
    getname() {
      localStorage.setItem("hometext", "【门诊补偿情况分析（医院）】");
    ...
    Chartselect(value) {
      ...
      if ("undefined" != typeof this.alldataRemake) {
        var dom = document.getElementById("cliContainter");
    ...
    getClistatic() {
      ...
      axios({
        method: "post",
        url: axios.PARK_API + "/xnhidc/backstage/loadddyy_menzhen",
        data: qs.stringify({
        ...
        if (res.data.success) {
          ...
          for (var i = 0; i < echarts_date.length; i++) {
            ...
            datamzzrenci[i] = echarts_date[i].门诊总人次;
            datamzzrenshu[i] = echarts_date[i].门诊总人数;
            datazylfeiyong[i] = echarts_date[i].门诊总医疗费用;
            datazrjfeiyong[i] = echarts_date[i].门诊人均费用;
            dataypfeiyong[i] = echarts_date[i].门诊药品费用;
            datazlfeiyong[i] = echarts_date[i].门诊诊疗费用;
            datazffeiyong[i] = echarts_date[i].门诊自费费用;
            datakbjine[i] = echarts_date[i].门诊可报金额;
            databxjine[i] = echarts_date[i].门诊报销金额;
            datafdrs[i] = echarts_date[i].门诊到达封顶人数;
          }
          ...
          for (var name in echarts_date[0]) {
            ...
            dataRemake.datamzzrenci = datamzzrenci;
            dataRemake.datamzzrenshu = datamzzrenshu;
    ...
    areaLoading() {
      this.flag = 2;
      var optionType;
      if (this.typeName == "门诊药品费用") {
        optionType = "yp";
      } else if (this.typeName == "门诊诊疗费用") {
        optionType = "zl";
      } else {
        return;
      }
      ...
      axios({
        method: "post",
        url: axios.PARK_API + "/xnhidc/backstage/loadddyy_menzhen_yaoping",
        data: qs.stringify({
          sdate: this.startTime,
          edate: this.endTime,
    ...
    townLoading(name) {
      ...
      axios({
        method: "post",
        url: axios.PARK_API + "/xnhidc/backstage/loadddyy_menzhen_yaoping_mx",
        data: qs.stringify({
          sdate: this.startTime,
          edate: this.endTime,
          yiyuanCode: this.areaCodeId,
