<template>
   <div>
      ...
      <div class="chart">
        ...
        <div id="hosContainter" :style="{width: '100%', height: '470px'}">
...
export default {
  ...
  methods: {
    getname() {
      localStorage.setItem("hometext", "【住院补偿情况分析（项目）】");
    ...
    getHostatic() {
      ...
      axios({
        method: "post",
        url: axios.PARK_API + "/xnhidc/compensate/hospitalization/queryProject",
        params: {
        ...
        if (res.data.code == 0) {
          ...
          for (var i = 0; i < echarts_date.length; i++) {
            dataAxis[i] = echarts_date[i].d50503;
      ...
      axios({
        ...
        if (res.data.code == 0) {
          ...
          for (var i = 0; i < echarts_date.length; i++) {
            ...
            ypmc[i] = echarts_date[i].d50505;
            zje[i] = echarts_date[i].fee;
            areacode[i] = echarts_date[i].d50503;
          }
          var option = {
            title: {
              text: name,
