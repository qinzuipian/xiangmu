<template>
   <div>
      <div class="right-ctn-title">
            ...
            <el-button size="mini" type="warning" style="margin-left: 5px;" @click="getClinic">
...
export default {
  ...
  methods: {
    getname() {
      localStorage.setItem("hometext", "【门诊补偿情况分析（地区）】");
    ...
    getClinic() {
      ...
      axios({
        method: "post",
        // url: axios.PARK_API + "/backstage/loadPicData_menzhen",
        url: axios.PARK_API + "/xnhidc/backstage/loadPicData_menzhen",
        data: qs.stringify({
        ...
        if (res.data.success) {
          ...
          for (var i = 0; i < echarts_date.length; i++) {
            ...
            datazyzrenshu[i] = echarts_date[i].门诊总人数;
